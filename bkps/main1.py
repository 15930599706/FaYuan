...
##数据库配置及全局变量##
conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='wasd1234', db='info3', charset='utf8')
...
path2 = "C:/Users/Think/Documents/Tencent Files/857720484/FileRecv/Beijing2/"


##文件的初始化##
def Initialize(path):
	global stra
	f = open(path, errors='ignore',encoding='utf8')  # 字符类型错误时忽略
	stra = f.read()
...
def Person(path, info):
	f = open(path, errors='ignore',encoding='utf8')
...
def CountWord(file, count):
	f = open(file, errors='ignore',encoding='utf8')
	count += 1
	print(count)
	word_lst = []
	key_list = []
